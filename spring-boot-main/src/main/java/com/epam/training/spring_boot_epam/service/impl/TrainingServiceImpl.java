...
public class TrainingServiceImpl implements TrainingService {
    ...
    public ApiResponse<Void> addTraining(TrainingDTO dto) {
        ...
        trainerWorkloadDTO.setUsername(trainer.getUser().getUsername());
        trainerWorkloadDTO.setFirstName(trainer.getUser().getFirstName());
        trainerWorkloadDTO.setLastName(trainer.getUser().getLastName());
        trainerWorkloadDTO.setActive(trainer.getUser().getActive());
        trainerWorkloadDTO.setMinutes(dto.getTrainingDurationInMinutes());
    ...
    @Transactional
    @Override
    public ApiResponse<Void> cancelTraining(Long trainingId) {
        LOGGER.info("Request to cancel training: {}", trainingId);

        Training training = trainingDao.findById(trainingId).orElseThrow(() -> new DomainException("Training not found"));

        if(training.getStatus().equals(DomainStatus.DELETED)){
            throw new DomainException("Training does not exist");
        }

        Trainer trainer = training.getTrainer();

        checkAuthProfile(trainer.getUser().getUsername());
        ...
        trainerWorkloadDTO.setLastName(trainer.getUser().getLastName());
        trainerWorkloadDTO.setActive(trainer.getUser().getActive());
        trainerWorkloadDTO.setMinutes(training.getTrainingDurationInMinutes());
        trainerWorkloadDTO.setActionType("CANCEL");
    ...
    @Override
    public ApiResponse<TrainerWorkloadResponseDTO> getTrainerWorkloadByMonth(String username) {
        ApiResponse<TrainerWorkloadResponseDTO> body = trainingWorkloadService.getTrainerWorkloadByMonth(username).getBody();
        if (!body.isSuccess()) {
            throw new ServiceUnavailableException(body.getMessage());
        }
